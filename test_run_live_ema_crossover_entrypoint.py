"""Tests for the EMA crossover Alpaca live runner entrypoint."""
...
import run_live_ema_crossover_strategy as runner
from run_live_ema_crossover_strategy import (
    create_ema_crossover_alpaca_config_from_env,
    create_ema_crossover_strategy_input_from_env,
    seconds_until_israel_stop,
    seconds_until_ny_regular_open,
    validate_ema_crossover_alpaca_config,
)
...
def test_create_ema_crossover_alpaca_config_reads_dedicated_env_names(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ALPACA_API_KEY", "shared-key")
    monkeypatch.setenv("ALPACA_SECRET_KEY", "shared-secret")
    monkeypatch.setenv("EMA_CROSSOVER_ALPACA_API_KEY", "ema-key")
    monkeypatch.setenv("EMA_CROSSOVER_ALPACA_SECRET_KEY", "ema-secret")
    monkeypatch.setenv("EMA_CROSSOVER_ALPACA_PAPER", "false")

    config = create_ema_crossover_alpaca_config_from_env()

    assert config.api_key == "ema-key"
    assert config.secret_key == "ema-secret"
    assert config.paper is False


def test_validate_ema_crossover_alpaca_config_requires_credentials(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("EMA_CROSSOVER_ALPACA_API_KEY", raising=False)
    monkeypatch.delenv("EMA_CROSSOVER_ALPACA_SECRET_KEY", raising=False)
    config = create_ema_crossover_alpaca_config_from_env()

    with pytest.raises(RuntimeError, match="EMA_CROSSOVER_ALPACA_API_KEY"):
        validate_ema_crossover_alpaca_config(config)


def test_create_ema_crossover_strategy_input_maps_env_names(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("EMA_CROSSOVER_PORTFOLIO_PCT_PER_TRADE", "0.2")
    monkeypatch.setenv("EMA_CROSSOVER_TRAILING_STOP_PCT", "0.03")
    monkeypatch.setenv("EMA_CROSSOVER_REWARD_TO_RISK", "3.0")
...
def test_create_ema_crossover_strategy_input_defaults_to_half_portfolio_split(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("EMA_CROSSOVER_PORTFOLIO_PCT_PER_TRADE", raising=False)
...
def test_main_wires_runner_dependencies_and_cleans_up(
    ...
    monkeypatch.setenv("EMA_CROSSOVER_ALPACA_API_KEY", "ema-key")
    monkeypatch.setenv("EMA_CROSSOVER_ALPACA_SECRET_KEY", "ema-secret")
    monkeypatch.setenv("EMA_CROSSOVER_FAST_PERIOD", "7")
    monkeypatch.setattr(runner, "MarketCalendar", FakeMarketCalendar)
    monkeypatch.setattr(runner, "AlpacaBroker", FakeBroker)
    monkeypatch.setattr(runner, "EmaCrossoverLiveStrategy", FakeStrategy)
    ...
    assert FakeStrategy.instances[0].kwargs["fast_period"] == 7
