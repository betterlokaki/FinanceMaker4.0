"""Run the EMA crossover strategy in live Alpaca mode."""
...
DEFAULT_PORTFOLIO_PCT_PER_TRADE = 0.5 / len(EmaCrossoverLiveStrategy.EMA_CROSSOVER_TICKERS)


def create_ema_crossover_alpaca_config_from_env() -> AlpacaConfig:
    """Create a separate Alpaca config from EMA_CROSSOVER_* env variables."""
    return AlpacaConfig(
        api_key=os.getenv("EMA_CROSSOVER_ALPACA_API_KEY", "").strip(),
        secret_key=os.getenv("EMA_CROSSOVER_ALPACA_SECRET_KEY", "").strip(),
        paper=_env_bool("EMA_CROSSOVER_ALPACA_PAPER", default=True),
        url_override=os.getenv("EMA_CROSSOVER_ALPACA_URL_OVERRIDE", "").strip(),
    )


def validate_ema_crossover_alpaca_config(config: AlpacaConfig) -> None:
    """Fail fast when dedicated EMA Alpaca credentials are missing."""
    if not config.api_key or not config.secret_key:
        raise RuntimeError(
            "Missing EMA crossover Alpaca credentials. Set "
            "EMA_CROSSOVER_ALPACA_API_KEY and EMA_CROSSOVER_ALPACA_SECRET_KEY."
        )


def create_ema_crossover_strategy_input_from_env() -> StrategyInputModel:
    """Create sizing input with half the portfolio split across the universe."""
    risk_pct = _env_float("EMA_CROSSOVER_TRAILING_STOP_PCT", 0.02)
    reward_to_risk = _env_float("EMA_CROSSOVER_REWARD_TO_RISK", 3.0)
    return StrategyInputModel(
        portfolio_pct_per_trade=_env_float(
            "EMA_CROSSOVER_PORTFOLIO_PCT_PER_TRADE",
...
async def main() -> None:
    """Start the isolated EMA crossover strategy runner."""
    alpaca_config = create_ema_crossover_alpaca_config_from_env()
    validate_ema_crossover_alpaca_config(alpaca_config)
    ...
    if wait_seconds > 0:
        ...
        if shutdown_task in done:
            logger.info("Shutdown requested before EMA crossover startup. Exiting.")
            return
    ...
    strategy_input = create_ema_crossover_strategy_input_from_env()
    strategy = EmaCrossoverLiveStrategy(
        realtime_provider=realtime_provider,
        market_provider=market_provider,
        broker=broker,
        strategy_input=strategy_input,
        fast_period=_env_int("EMA_CROSSOVER_FAST_PERIOD", 9),
        slow_period=_env_int("EMA_CROSSOVER_SLOW_PERIOD", 21),
        candle_minutes=_env_int("EMA_CROSSOVER_CANDLE_MINUTES", 5),
        trailing_stop_pct=_env_float("EMA_CROSSOVER_TRAILING_STOP_PCT", 0.02),
        reward_to_risk=_env_float("EMA_CROSSOVER_REWARD_TO_RISK", 3.0),
    )
    ...
    try:
        ...
        logger.info("EMA crossover strategy running with tickers: %s", strategy.tickers)
        ...
        logger.info("EMA crossover runner shutdown complete")
...
async def wait_until_stop_or_shutdown(shutdown_event: asyncio.Event) -> None:
    ...
    if stop_task in done:
        logger.info("Reached 23:00 Israel time. Stopping EMA crossover strategy...")
    if shutdown_task in done:
        logger.info("Shutdown requested. Stopping EMA crossover strategy...")
    for task in pending:
        task.cancel()
    await asyncio.gather(*pending, return_exceptions=True)
