#!/usr/bin/env python3
"""Nested validation for Mag7 5-minute exhaustion-reversal strategy."""
...
from backtests.backtesting_py.mag7_intraday_exhaustion_reversal_strategy import (
    ExhaustionReversalParams,
    run_exhaustion_reversal_portfolio,
)
...
DEFAULT_OUTPUT_PATH = (
    PROJECT_ROOT / "backtests" / "results" / "mag7_intraday_exhaustion_reversal_backtest.json"
)
...
def main() -> int:
    ...
    print("MAG7 5-MINUTE EXHAUSTION REVERSAL NESTED VALIDATION", flush=True)
    print("=" * 112, flush=True)
    ...
    print(f"Scoring {len(candidates)} exhaustion-reversal candidates...", flush=True)
    scored: list[dict[str, Any]] = []
    progress_interval = max(1, min(25, max(5, len(candidates) // 4)))

    for idx, params in enumerate(candidates, start=1):
        subtrain = run_exhaustion_reversal_portfolio(
        ...
        validation = run_exhaustion_reversal_portfolio(
            data_by_ticker=strategy_dev_data,
            tickers=tickers,
            params=params,
            initial_capital=initial_capital,
    ...
    best_params = ExhaustionReversalParams(**scored[0]["params"])
    full_dev = run_exhaustion_reversal_portfolio(
        data_by_ticker=strategy_dev_data,
        tickers=tickers,
        params=best_params,
        initial_capital=initial_capital,
    ...
    holdout = run_exhaustion_reversal_portfolio(
    ...
    payload = {
        "strategy": "mag7_intraday_exhaustion_reversal",
        "tickers": tickers,
...
def _candidate_params(*, max_candidates: int) -> list[ExhaustionReversalParams]:
    rng = random.Random(20260621)
    values = {
        "failure_bars": (3, 6, 9),
        "atr_bars": (14, 20, 28),
        "exhaustion_atr_mult": (1.0, 1.5, 2.0, 2.5),
        "failure_atr_fraction": (0.0, 0.1, 0.2, 0.3),
        "use_volume_filter": (False, True),
        "volume_lookback": (20, 39),
        "volume_multiple": (1.0, 1.2, 1.5, 2.0),
        "stop_atr_buffer": (0.0, 0.2, 0.4, 0.6),
    ...
    out: list[ExhaustionReversalParams] = []
    ...
    while len(out) < max_candidates and attempts < max_candidates * 100:
        ...
        if key in seen:
            continue
        seen.add(key)
        out.append(ExhaustionReversalParams(**choice))
    return out
...
def _nested_score(*, sub_summary: dict[str, Any], val_summary: dict[str, Any]) -> float:
    ...
    rr_penalty = max(0.0, 1.8 - realized_rr) * 4.0
