#!/usr/bin/env python3
"""Nested validation for Mag7 5-minute cross-sectional long/short strategy."""
...
from backtests.backtesting_py.mag7_intraday_cross_section_strategy import (
    CrossSectionParams,
    run_cross_section_sleeve_portfolio,
)
...
DEFAULT_OUTPUT_PATH = (
    PROJECT_ROOT / "backtests" / "results" / "mag7_intraday_cross_section_backtest.json"
)
...
def main() -> int:
    ...
    print("MAG7 5-MINUTE CROSS-SECTION NESTED VALIDATION", flush=True)
    print("=" * 112, flush=True)
    ...
    print(f"Scoring {len(candidates)} cross-section candidates...", flush=True)
    scored: list[dict[str, Any]] = []
    progress_interval = max(1, min(25, max(5, len(candidates) // 4)))

    for idx, params in enumerate(candidates, start=1):
        subtrain = run_cross_section_sleeve_portfolio(
        ...
        validation = run_cross_section_sleeve_portfolio(
            data_by_ticker=strategy_dev_data,
            tickers=tickers,
            params=params,
            initial_capital=initial_capital,
        ...
        if idx % progress_interval == 0 or idx == len(candidates):
            ...
            print(
                ...
                f"val dd={leader['validation']['max_drawdown_pct']:+.2f}% | "
    ...
    best_params = CrossSectionParams(**scored[0]["params"])
    full_dev = run_cross_section_sleeve_portfolio(
        data_by_ticker=strategy_dev_data,
        tickers=tickers,
        params=best_params,
        initial_capital=initial_capital,
    ...
    holdout = run_cross_section_sleeve_portfolio(
    ...
    payload = {
        "strategy": "mag7_intraday_cross_section",
        "tickers": tickers,
...
def _candidate_params(*, max_candidates: int) -> list[CrossSectionParams]:
    rng = random.Random(20260621)
    values = {
        "style": ("momentum", "reversal"),
        "fast_bars": (3, 6, 12),
        "mid_bars": (12, 24, 39),
        "slow_bars": (39, 78, 156),
        "fast_weight": (0.5, 1.0, 1.5),
        "mid_weight": (0.5, 1.0, 1.5),
        "slow_weight": (0.0, 0.5, 1.0),
        "vwap_weight": (0.0, 0.5, 1.0),
        "open_weight": (0.0, 0.5, 1.0),
        "vol_adjust": (False, True),
        "long_count": (1, 2, 3),
        "short_count": (1, 2, 3),
        "use_vwap_side_filter": (False, True),
        "use_dispersion_filter": (True,),
        "dispersion_lookback_bars": (6, 12, 24, 36),
        "dispersion_window_bars": (120, 240, 480),
        "min_dispersion_quantile": (0.6, 0.7, 0.8),
        "entry_start_bar": (6, 9, 12),
        "leverage": (1.5, 2.0, 2.5, 3.0),
        "stop_pct": (0.004, 0.006, 0.008, 0.01, 0.012),
        "risk_reward_ratio": (2.0, 2.5, 3.0),
        "max_holding_bars": (12, 24, 39, 78),
        "max_trades_per_day": (1, 2, 4),
    }
    out: list[CrossSectionParams] = []
    ...
    while len(out) < max_candidates and attempts < max_candidates * 100:
        ...
        if int(choice["fast_bars"]) >= int(choice["mid_bars"]):
            continue
        if int(choice["mid_bars"]) >= int(choice["slow_bars"]):
            continue
        choice["min_abs_score"] = rng.choice(
            (0.0, 0.25, 0.5, 1.0, 1.5)
            if bool(choice["vol_adjust"])
            else (0.0, 0.001, 0.002, 0.004, 0.006)
        )
        key = tuple(choice.items())
        if key in seen:
            continue
        seen.add(key)
        out.append(CrossSectionParams(**choice))
    return out
...
def _nested_score(*, sub_summary: dict[str, Any], val_summary: dict[str, Any]) -> float:
    ...
    rr_penalty = max(0.0, 1.6 - realized_rr) * 5.0
    return robust * 3.0 + val_mean + val_min_iso + pf + realized_rr + hit_ratio * 4.0 - dd / 3.0 - rr_penalty
