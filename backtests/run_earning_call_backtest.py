#!/usr/bin/env python3
"""Run earning call strategy backtest with batch visualization.

Runs backtest on tickers in batches of 10, showing each batch's equity curve
against S&P 500, then shows final combined results.
"""
...
# Scanner URL for earnings stocks
SCANNER_URL = (
    "https://finviz.com/screener.ashx?v=111&f="
    "earningsdate_thismonth%2Csh_avgvol_o1000&ft=4"
)
...
async def fetch_tickers() -> list[str]:
    """Fetch tickers from Finviz scanner."""
    async with httpx.AsyncClient() as client:
        scanner = CustomFinviz(http_client=client, url=SCANNER_URL)
        return await scanner.scan(ScannerParams("earnings"))
...
def plot_final_comparison(
    ...
    ax.plot(
        ...
        label="Earning Call Strategy (All Tickers)",
        color="#2ecc71",
        linewidth=3,
    )
    ...
    ax.set_title(
        f"Earning Call Strategy - Final Results ({total_tickers} Tickers)\n"
        f"Entry: Day's Low Before Earnings | Risk:Reward = 4%:8%",
        fontsize=16,
        fontweight="bold",
    )
...
def main() -> None:
    """Run earning call strategy backtest in batches."""
    print("=" * 60)
    print("EARNING CALL STRATEGY BACKTEST")
    print("Entry: Day's Low Before Earnings")
    print("Risk:Reward = 4%:8%")
    print("=" * 60)
    
    # Fetch tickers
    print("\n1. Fetching earnings tickers from Finviz...")
    ...
    params = BacktestParams(
        ...
        end_date=end_date,
        interval="1d",  # Daily data for earnings strategy
    )
    ...
    for i in range(0, len(tickers), BATCH_SIZE):
        ...
        strategy = EarningCallStrategy()
        engine = EarningCallEngine(strategy=strategy)
